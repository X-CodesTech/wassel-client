...
export default function CustomerDetails() {
  ...
  useEffect(() => {
    if (match && params.id) {
      ...
      if (redirectToList) {
        ...
        const timer = setTimeout(() => setLocation("/price-lists?tab=customers"), 0);
  ...
  const handleExportCSV = () => {
    ...
    const rows = customer.priceItems.map(item => 
      `"${item.subActivityName}",${item.price}`
    ).join("\n");
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          {customer.contactPerson && (
            <p className="text-gray-500 mt-1">Contact: {customer.contactPerson}</p>
          )}
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            This customer has {customer.priceItems.length} items with custom pricing
          </CardDescription>
        </CardHeader>
        <CardContent>
          {customer.priceItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-gray-500 max-w-md">
                This customer doesn't have any custom prices yet.
                You can import prices using the "Import Prices" button.
              </p>
            </div>
          ) : (
            <Table>
              <TableCaption>List of custom prices for {customer.name}</TableCaption>
              <TableHeader>
                <TableRow>
      ...
      {/* File Upload Dialog */}
      <AlertDialog open={fileUploadDialogOpen} onOpenChange={setFileUploadDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Import Prices from CSV</AlertDialogTitle>
            <AlertDialogDescription>
              Upload a CSV file with custom prices for this customer.
              The file should have two columns: Sub-Activity Name and Price.
            </AlertDialogDescription>
          </AlertDialogHeader>
